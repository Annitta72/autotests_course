...
class PersonInfo:
    """
    Класс описывающий персональную информацию сотрудника
    """

    def __init__(self, name: str, age: int, *dep):
        """
        Конструктор с обязательными атрибутами класса - персональная информация сотрудника

        :param name: входная строка - Имя Фамилия

        :param age: входное число (int) - возраст

        :param dep: входной список - подразделения
        """
        self.name = name
        self.age = age
        self.dep = dep
    ...
    def short_name(self):
        ...
        :return: результирующая строка Фамилия И.
        """
        lst_name = self.name.split(' ')
        return f'{lst_name[-1]} {lst_name[0][:1]}.'

    def path_deps(self):
        """
        Метод, который возвращает путь "Головное подразделение --> ... --> Конечное подразделение"

        :return: результирующая строка "Головное подразделение --> ... --> Конечное подразделение"
        """
        return ' --> '.join(self.dep)

    def new_salary(self):
        """
        Метод индексации зарплаты по формуле:
        1337*Возраст*суммарное кол-во вхождений трех наиболее часто встречающихся букв из списка подразделений

        :return: результирующее число - проиндексированная зарплата
        """
        s = ''.join(filter(str.isalpha, str(self.dep)))
        letters_dict = {}
        for x in s:
            letters_dict[x] = letters_dict.get(x, 0) + 1
        summa = sum((sorted(letters_dict.values(), reverse=True))[:3])
        form = 1337 * self.age * summa
        return form

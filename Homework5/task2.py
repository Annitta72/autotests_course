...
def repeats(our_str):
    """
    Функция, которая смотрит, сколько раз каждый символ уже встречался,
    и добавляет в результирующую строку количество к символам с помощью постфикса формата _n.
    Args:
        our_str (str): входная строка

    Returns:
        str: результирующая строка количество к символам с помощью постфикса формата _n
    """
    new_str = ''
    letters_dict = {}
    for lt in our_str:
        letters_dict[lt] = letters_dict.get(lt, 0) + 1
        new_str += f'{lt}_{letters_dict[lt]}'
    return new_str

...
def josephus_task(num_people, kill_num):
    ...
    survivor = 0  # Создание и присваивание конечной позиции "выживший" значения 0 (индекс)
    # Перебираем циклом сгенерированный range список от 1 до кол-ва воинов
    # (к кол-ву воинов + 1, т.к. иначе последнее число не попадет в список)
    for x in range(1, num_people + 1):
        # Нахождение позиции выжившего (индекс) на предыдущем шаге (позиция выжившего + позиция вышедшего)
        # (и % на кол-во не выбывших воинов в круге)
        survivor = (survivor + kill_num) % x
    survivor += 1  # Находим предыдущую позицию выжившего (+ 1, т.к. индексы отчитываются с 0)
    return survivor

...
def scrabble(word):
    """
    Функция, которая считает кол-во очков, полученное за слово
    Args:
        word (str): входящая строка (слово)

    Returns:
        int: результирующее число (кол-во очков)
    """
    # Здесь нужно написать код
    dict_points = {1: 'авеёинорст', 2: 'дклмпу', 3: 'бгья', 4: 'йы', 5: 'жзхцч', 8: 'фшэю', 10: 'щ', 15: 'ъ'}
    points = (sum([k for x in word for k, v in dict_points.items() if x in v]))
    return points
